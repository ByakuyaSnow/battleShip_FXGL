...
import fi.utu.tech.gui.javafx.subscene.BluePlayerDeployScene;
import fi.utu.tech.gui.javafx.subscene.BluePlayerScene;
import fi.utu.tech.gui.javafx.subscene.RedPlayerDeployScene;
import fi.utu.tech.gui.javafx.subscene.RedPlayerScene;
import fi.utu.tech.gui.javafx.userAction.AttackAction;
import fi.utu.tech.gui.javafx.userAction.DeployAction;
...
public class BattleShipApp extends GameApplication {
    ...
    protected void initUI() {
        ...
        runOnce(()->{
            getDialogService().showMessageBox("Welcome game,\nleft click to attack,right click to deploy,\n you can change scene with SPACE");
        },Duration.ZERO);
    ...
    protected void initInput() {
        ...
        onBtnDown(MouseButton.SECONDARY,()->{
            var entityList = getGameWorld().getEntitiesByComponent(DraggableComponent.class);
            entityList.forEach(entity ->{
                if(entity.getComponent(DraggableComponent.class).isDragging()){
                    this.captureEntity = entity;
                    Point2D pos = getCloseAnchorPoint(entity.getPosition());
                }
            } );
        });
         */
    ...
    private void nextScene() {
        ...
        int blueTeamRemain = getGameWorld().getEntitiesByComponent(BlueTeamComponent.class).size();
        int redTeamRemain = getGameWorld().getEntitiesByComponent(RedTeamComponent.class).size();

        if(blueTeamRemain<=0){
            showGameOver();
            getDialogService().showMessageBox("BLUE TEAM LOSS");
        }

        if(redTeamRemain<=0){
            showGameOver();
            getDialogService().showMessageBox("RED TEAM LOSS");
        }
    ...
    private void setupShipEntity(){
        ...
        var carrierNum = BattleShipApp.carrierNum;
        var battleshipNum = BattleShipApp.battleShipNum;
        var cruiserNum = BattleShipApp.cruiserNum;
        var submarineNum = BattleShipApp.submarineNum;
        var destroyerNum = BattleShipApp.destroyerNum;
